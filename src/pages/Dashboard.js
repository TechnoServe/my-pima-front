...
const Dashboard = ({ trainingGroups, trainingSessions, participants, projectStats }) => {
  ...
  const openList = (e, name) => {
    ...
    if (name === 'total_bas') {
      ...
      setList([...new Set(trainingGroups.map((group) => group.business_advisor))])
    }
    if (name === 'total_fts') {
      setOpen(true)
      setTitle('Farmer Trainers')
      setList([...new Set(trainingGroups.map((group) => group.farmer_trainer))])
    }
  }
    ...
    <div>
      ...
      >
        {statsData.map((data, index) => (
          <div key={index}>
            <NavLink to={data.path ? data.path : null} key={index} onClick={(e) => (data.path ? null : openList(e, data.name))}>
              <Card heading={data.heading} figures={data.figures} icon={data.icon} color={data.color} />
            </NavLink>
          </div>
        ))}{' '}
      </div>
      ...
      <ListUpModal open={trainingGroups.length > 0 ? open : false} handleClose={() => setOpen(false)} data={list} title={title} />
    </div>
  )
}
