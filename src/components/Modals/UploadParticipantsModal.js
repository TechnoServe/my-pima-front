...
import { Dialog, DialogContent, Typography, Button, Box, DialogActions, Chip, Alert } from '@mui/material'
...
const UploadParticipantsModal = ({ open, setOpen, navigatedProject }) => {
  ...
  useEffect(() => {
    ...
    const projectColumn = fileInfo.data.map((row) => row.split(',')[loadedColumns.indexOf('Project')])
        ...
        !distinctProjects.includes(navigatedProject) ? (
          ...
          >
            ...
            >
              ...
              />
              The project(s) in the file does not match the project you are currently navigating. Please upload a file with the correct
              project.
            </Typography>
            <div className='upload_actions'>
              <AiOutlineCloseCircle
              ...
              <em>
                {
                  ...
                  (fileInfo.data.length - 2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                }
              </em>
            </Typography>
                    ...
                    {`(
                      ${
                        fileInfo.data.filter((row) => {
                          const rowProject = row.split(',')[loadedColumns.indexOf('Project')]
