...
import { MdOutlineDashboardCustomize, MdOutlineGroups, MdOutlineCalendarToday, MdOutlinePersonSearch, MdLogout } from 'react-icons/md'
    ...
    <div className='nav__container'>
      ...
      >
        ...
        <div className='bottom__section'>
          <NavLink to={bottomitem[0].path} className='link' activeclassname='active'>
            <div>{bottomitem[0].icon}</div>
            <div>
              <div style={{ display: isOpen ? 'block' : 'none' }} className='profile__text'>
                {bottomitem[0].name}
              </div>{' '}
              <div style={{ display: isOpen ? 'block' : 'none' }} className='profile__text'>
                {bottomitem[0].email}
          ...
          <NavLink to={bottomitem[1].path} className='link' activeclassname='active' onClick={handleLogout}>
      ...
      <main style={{ width: '100%', marginLeft: isOpen ? '220px' : '60px' }}>{children}</main>
    </div>
  )
}
