...
const Partdetail = ({ participants, trainingSessions }) => {
  ...
  const selectedParticipant = participants && participants.find((participant) => participant.p_id === id)

  return (
    <>
      <Breadcrumb name='Details' firstItem={breadCrumbs} linkTo={breadCrumbsLinkTo} />
          ...
          selectedParticipant ? (
            <>
              ...
              <div className='parts__tablecontent'>
                <Partstableview trainingSessions={trainingSessions} participant={selectedParticipant} />
              </div>
            </>
          ) : (
